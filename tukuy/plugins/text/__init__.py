...
This is the canonical home for all text transformer classes and the
TextTransformersPlugin that exposes them to the Tukuy registry.
"""
...
@register_transformer("template", template="")
...
@register_transformer("split", delimiter=":", index=-1)
...
@register_transformer("title_case")
...
@register_transformer("camel_case")
...
@register_transformer("snake_case")
...
@register_transformer("remove_emojis")
...
@register_transformer("redact_sensitive")
...
class TextTransformersPlugin(TransformerPlugin):
    ...
    def transformers(self):
        ...
        result = self._auto_transformers()
        # Manual entries for classes imported from base (not in this module)
        result['regex'] = lambda params: RegexTransformer('regex',
            pattern=params.get('pattern', ''),
            template=params.get('template'))
        result['replace'] = lambda params: ReplaceTransformer('replace',
            old=params.get('from', ''),
            new=params.get('to', ''))
        # map needs param rename: 'values' -> 'mapping'
        result['map'] = lambda params: MapTransformer('map',
